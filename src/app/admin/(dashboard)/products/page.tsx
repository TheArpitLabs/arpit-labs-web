...
export default async function AdminProductsPage({ searchParams }: AdminProductsPageProps) {
  ...
  return (
    <div className="space-y-6">
      <AdminTopbar
        title="Products CMS"
        subtitle="Create, manage, and publish product offerings for the Axiora suite."
      />
      ...
      <div className="flex flex-col gap-6 lg:flex-row">
        ...
        <div className="w-full lg:w-3/5 space-y-4">
          <AdminSection
            ...
            description="View published products, plan updates, and featured offerings."
          >
            <div className="mb-6 flex flex-wrap gap-4">
              <form className="flex flex-1 items-center gap-2">
                ...
                {params && Object.keys(params).length > 0 && (
                  <Link
                    href="/admin/products"
                    className="flex h-10 items-center justify-center rounded-xl border border-border/70 px-4 text-sm font-medium"
                  >
                    Clear
                  </Link>
                )}
            ...
            {products.length > 0 ? (
              <AdminTable headers={['Product', 'Status', 'Featured', 'Actions']}>
                {products.map((product) => (
                  <tr key={product.id} className="border-b border-border/40 last:border-0">
                    ...
                    <td className="px-4 py-4">
                      <span
                        className={`inline-flex rounded-full px-2 py-1 text-[10px] font-bold uppercase tracking-wider ${product.published ? 'bg-green-500/10 text-green-500' : 'bg-yellow-500/10 text-yellow-500'}`}
                      >
              ...
              <AdminEmptyState
                ...
                description="Create a new product to publish it to the hub."
              />
            )}
          </AdminSection>
        </div>
