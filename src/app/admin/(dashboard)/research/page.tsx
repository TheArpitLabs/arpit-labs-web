...
export default async function AdminResearchPage({ searchParams }: AdminResearchPageProps) {
  ...
  const editingPaper = params?.edit ? papers.find((p) => p.id === params.edit) : null;
      ...
      <div className="flex flex-col gap-6 lg:flex-row">
        <div className="w-full lg:w-2/5">
          <AdminSection
            ...
            description="Publish new research to the Axiora ecosystem."
          >
          ...
          <AdminSection title="Publications" description="Your published research library.">
            {papers.length > 0 ? (
              <AdminTable headers={['Title', 'Division', 'Featured', 'Actions']}>
                {papers.map((paper) => (
                  <tr key={paper.id} className="border-b border-border/40 last:border-0">
                    ...
                    <td className="px-4 py-4">{paper.is_featured ? '★' : '—'}</td>
                    <td className="px-4 py-4">
              ...
              <AdminEmptyState
                ...
                description="Start by publishing your first research paper."
              />
            )}
          </AdminSection>
        </div>
