  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!data.session && data.user) {
        setSuccess("Please check your email to confirm your account. The link will expire in 24 hours.");
        setLoading(false);
    ...
    <main className="min-h-screen bg-gradient-to-br from-slate-900 via-surface to-slate-900">
      ...
      <div className="px-4 py-10 text-foreground sm:px-6 lg:px-8">
        ...
        <div className="mb-8 space-y-3 text-center">
          <p className="text-xs font-heading font-semibold uppercase tracking-[0.28em] text-primary">Get Started</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-5" noValidate>
          <div className="space-y-2">
            <label htmlFor="fullName" className="text-sm font-heading font-medium text-foreground">Full name</label>
            <input
              ref={nameRef}
              id="fullName"
              type="text"
            ...
            {password && (
              <div className="space-y-2 mt-3">
                <div className="flex items-center justify-between">
                  <span className={`text-xs font-semibold ${getStrengthColor()}`}>Password strength: {getStrengthLabel()}</span>
                  <span className={`text-xs font-semibold ${getStrengthColor()}`}>{strengthScore}/5</span>
                </div>
                <div className="space-y-1">
                  ...
                  <div className="flex items-center gap-2 text-xs">
                    <span className={passwordStrength.hasLower ? "text-green-400" : "text-gray-500"}>
                      {passwordStrength.hasLower ? <Check className="h-3 w-3 inline" /> : <X className="h-3 w-3 inline" />}
                    </span>
                    <span className={passwordStrength.hasLower ? "text-green-400" : "text-gray-500"}>Contains lowercase letter</span>
                  </div>
                  <div className="flex items-center gap-2 text-xs">
                    <span className={passwordStrength.hasSpecial ? "text-green-400" : "text-gray-500"}>
                      {passwordStrength.hasSpecial ? <Check className="h-3 w-3 inline" /> : <X className="h-3 w-3 inline" />}
                    </span>
                    <span className={passwordStrength.hasSpecial ? "text-green-400" : "text-gray-500"}>Contains special character</span>
                  </div>
                </div>
              </div>
            )}
        ...
        <div className="mt-6 text-center">
          <p className="text-sm text-muted">
            ...
            <Link href="/login" className="font-heading font-medium text-primary hover:text-accent transition">
              Sign in
            </Link>
          </p>
        </div>
