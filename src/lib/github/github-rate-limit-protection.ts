...
class GitHubRateLimitProtection {
  ...
  async checkRateLimit(): Promise<RateLimitCheckResult> {
    ...
    if (!status) {
      ...
      logger.warn('[RATE LIMIT] No rate limit data available, proceeding with caution');
      return { allowed: true };
    }
    ...
    if (status.remaining <= this.options.stopThreshold) {
      ...
      logger.error(`[RATE LIMIT] Rate limit exhausted. Waiting ${waitTimeMs}ms until reset`);
    ...
    if (status.remaining <= this.options.pauseThreshold) {
      ...
      logger.warn(`[RATE LIMIT] Rate limit at threshold. Pausing for ${waitTimeMs}ms`);
  ...
  private pauseDiscovery(durationMs: number): void {
    ...
    this.isPaused = true;
    logger.info(`[RATE LIMIT] Pausing discovery for ${durationMs}ms`);
  ...
  private resumeDiscovery(): void {
    ...
    logger.info('[RATE LIMIT] Resuming discovery');
  }
  ...
  pause(): void {
    ...
    this.isPaused = true;
    logger.info('[RATE LIMIT] Discovery manually paused');
