...
export async function logValidationEvent(entry: ValidationLogEntry) {
  try {
    ...
    console.error('Error in logValidationEvent:', error);
  }
}
...
export async function getProjectValidationHistory(projectId: string) {
  try {
    ...
    if (error) {
      console.error('Error fetching project validation history:', error);
      return [];
    }

    return logs || [];
  } catch (error) {
    console.error('Error in getProjectValidationHistory:', error);
    return [];
  }
}
...
async function ensureDiscoveryLogsTable() {
  try {
    ...
    if (error && error.code === '42P01') {
      console.log('Creating discovery_logs table...');
      await createDiscoveryLogsTable();
    }
  } catch (error) {
    console.error('Error ensuring discovery_logs table:', error);
  }
}
...
async function createDiscoveryLogsTable() {
  try {
    ...
    if (error) {
      console.error('Error creating discovery_logs table:', error);
    }
  } catch (error) {
    console.error('Error in createDiscoveryLogsTable:', error);
  }
}
