...
class TrendingEngine {
  ...
  async getTrendingByDomain(
    ...
    if (error) {
      logger.error('Error getting trending by domain:', error);
      return [];
    }
  ...
  async getTrendingBySubdomain(
    ...
    if (error) {
      logger.error('Error getting trending by subdomain:', error);
      return [];
    }
  ...
  async recalculateAllTrendingScores(): Promise<void> {
    ...
    if (error) {
      logger.error('Error recalculating trending scores:', error);
      throw error;
    }
  }
