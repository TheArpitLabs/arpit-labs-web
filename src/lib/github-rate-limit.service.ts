...
class GitHubRateLimitService {
  ...
  async fetchRateLimitStatus(): Promise<void> {
    try {
      ...
      if (!response.ok) {
        console.error('Failed to fetch rate limit status:', response.statusText);
        return;
      }
  ...
  private logRateLimitStatus(type: 'core' | 'search', status: RateLimitStatus): void {
    if (status.remaining <= this.CRITICAL_THRESHOLD) {
      console.error(`[CRITICAL] GitHub ${type} rate limit: ${status.remaining}/${status.limit} remaining. Reset at ${status.resetDate.toISOString()}`);
    } else if (status.remaining <= this.WARNING_THRESHOLD) {
      console.warn(`[WARNING] GitHub ${type} rate limit: ${status.remaining}/${status.limit} remaining. Reset at ${status.resetDate.toISOString()}`);
    } else {
      console.log(`[INFO] GitHub ${type} rate limit: ${status.remaining}/${status.limit} remaining. Reset at ${status.resetDate.toISOString()}`);
    }
  }
  ...
  updateFromResponseHeaders(headers: Headers): void {
    ...
    if (limit && used && remaining && reset) {
      ...
      if (status.remaining <= this.CRITICAL_THRESHOLD) {
        console.error(`[CRITICAL] GitHub rate limit (from headers): ${status.remaining}/${status.limit} remaining. Reset at ${status.resetDate.toISOString()}`);
      } else if (status.remaining <= this.WARNING_THRESHOLD) {
        console.warn(`[WARNING] GitHub rate limit (from headers): ${status.remaining}/${status.limit} remaining. Reset at ${status.resetDate.toISOString()}`);
      } else {
        console.log(`[INFO] GitHub rate limit (from headers): ${status.remaining}/${status.limit} remaining. Reset at ${status.resetDate.toISOString()}`);
      }
    }
  }
