...
export function DashboardSidebar({ user, profile, onClose }: DashboardSidebarProps) {
  ...
  const navigation = [
    ...
    { name: "Notifications", href: "/dashboard" as const, icon: Bell, badge: notifications, danger: true },
        ...
        {navigation.map((item) => {
          ...
          return (
            ...
            >
              ...
              {item.badge ? (
                <span className={`rounded-md px-1.5 py-0.5 text-[10px] font-bold text-white ${item.danger ? "bg-rose-500" : "bg-indigo-500"}`}>
                  {item.badge}
                </span>
              ) : null}
            </Link>
      ...
      <div className="mt-5 rounded-xl border border-white/10 bg-white/[0.03] p-3">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            <p className="truncate text-sm font-semibold text-white">{profile?.full_name || user?.email?.split("@")[0] || "Engineer"}</p>
